...
const BlogCard: React.FC<BlogCardProps> = ({ blog, onClick }) => {
  ...
  useEffect(() => {
    ...
    const fetchBlogImage = async () => {
      ...
      try {
        console.log(`Fetching image for mediaId: ${media.mediaId}`);
        
        // Use the correct photoFileApi.getMedia method for blog media
        const response = await photoFileApi.getPhotoPreview(media.mediaId);
