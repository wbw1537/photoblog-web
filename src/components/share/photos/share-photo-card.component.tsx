...
const PhotoCard: React.FC<PhotoCardProps> = ({ photo, onClick, sharedUserId }) => {
  ...
  useEffect(() => {
    ...
    const fetchImage = async () => {
      ...
      try {
        // Use the private photoFileApi method with sharedUserId
        const response = await photoFileApi.privateGetPhotoPreview(photo.files[0].id, sharedUserId);
