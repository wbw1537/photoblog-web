...
import ShareBlogGallery from '@/components/share/blogs/share-blog-gallery.component';
...
import { SharedUserResponse } from '@/types/shared-user.type';
import Image from 'next/image';

const ShareBlogsPage: React.FC = () => {
  ...
  // Fetch shared user and check API connectivity when component mounts
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        // Get the first shared user's ID
        const sharedUsersResponse = await sharedUserApi.getSharedUsers({ skip: 0, take: 1 });
        if (sharedUsersResponse.data.data && 
            (Array.isArray(sharedUsersResponse.data.data) ? 
             sharedUsersResponse.data.data.length > 0 : 
             sharedUsersResponse.data.data)) {
          
          const firstUser = Array.isArray(sharedUsersResponse.data.data) ? 
                           sharedUsersResponse.data.data[0] : 
                           sharedUsersResponse.data.data as SharedUserResponse;
                           
          setSharedUserId(firstUser.id);
        } else {
          setError(t('share.noSharedUserFound'));
        }
        setError(null);
      } catch (err) {
        logError(err);
  ...
  useEffect(() => {
    if (!selectedBlog || !selectedBlog.blogMedia || selectedBlog.blogMedia.length === 0 || !sharedUserId) return;
    ...
    const fetchMedia = async (mediaId: string) => {
      ...
      try {
        console.log(`Fetching detail image for mediaId: ${mediaId} from shared user ID: ${sharedUserId}`);
        // Use privateGetPhotoPreview instead of public getMedia
        const response = await photoFileApi.privateGetPhotoPreview(mediaId, sharedUserId);
  ...
  }, [selectedBlog, mediaUrls, loadingMedia, sharedUserId]);
  ...
  if (!sharedUserId) {
    return (
      <div className="flex h-screen items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-amber-600 mb-4">{t('share.noSharedUserFound')}</h2>
          <p className="text-gray-600 mb-6">{t('share.addSharedUserFirst')}</p>
        </div>
      </div>
    );
  }
